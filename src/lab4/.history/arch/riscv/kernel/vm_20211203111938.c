...
void create_mapping(uint64 *pgtbl, uint64 va, uint64 pa, uint64 sz, int perm) {
    ...
    */
   printk("create_mapping: pgtbl: %x, va: %x, pa: %x, sz: %x, perm: %d\n", pgtbl, va, pa, sz, perm);
   ...
   for(int i = 0; i < page_num; i++) {
        ...
        printk("curva = %x, curpa = %x\n", curva, curpa);
        printk("vpn2 = %x, vpn1 = %x, vpn0 = %x\n", vpn2, vpn1, vpn0);
        printk("ppn2 = %x, ppn1 = %x, ppn0 = %x\n", ppn2, ppn1, ppn0);

        uint64 first_page_addr = (uint64)pgtbl + (vpn2 << 3);
        
        printk("first_page_addr = %x\n", first_page_addr);
        
        uint64 first_page_entry;
        first_page_entry = read_mem((uint64*)first_page_addr);
        
        printk("first_page_entry = %x\n", first_page_entry);
