...
void setup_vm_final(void) {
    ...
    create_mapping((uint64*)swapper_pg_dir, VM_START + OPENSBI_SIZE, PHY_START + OPENSBI_SIZE, PHY_SIZE - OPENSBI_SIZE, 3);

    // mapping kernel rodata -|-|R|V
    create_mapping((uint64*)swapper_pg_dir, VM_START + OPENSBI_SIZE + (uint64)_srodata - (uint64)_stext, PHY_START + OPENSBI_SIZE + (uint64)_srodata - (uint64)_stext, );
...
create_mapping(uint64 *pgtbl, uint64 va, uint64 pa, uint64 sz, int perm) {
   ...
   for(int i = 0; i < page_num; i++) {
        ...
        if(!(first_page_entry & 0x1)) {
            first_page_entry = write_piece(first_page_entry, kalloc() >> 12, 53, 10);
