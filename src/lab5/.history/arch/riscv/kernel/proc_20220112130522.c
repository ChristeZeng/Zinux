...
void task_init() {
    ...
    for(int i = 1; i < NR_TASKS; i++){
        ...
        //?
        task[i]->thread.sstatus = (uint64)csr_read(sstatus) & (uint64)SPP | (uint64)SPIE;
        task[i]->thread.sstatus |= (uint64)SUM;
        ...
        create_mapping(task[i]->pgd, USER_START, (uint64)uapp_start - PA2VA_OFFSET, (uint64)uapp_end - (uint64)uapp_start, 0b111);
        create_mapping(task[i]->pgd, USER_END - PGSIZE, (uint64)kalloc() - PA2VA_OFFSET, PGSIZE, 0b1011);

        task[i]->pgd = (pagetable_t)(((csr_read(satp) >> 44) << 44) | ( ( (uint64)task[i]->pgd - PA2VA_OFFSET) >> 12));
