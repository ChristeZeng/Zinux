...
void create_user_mapping2(uint64 *pgtbl, uint64 va, uint64 pa, uint64 sz, int perm){
    ...
    for(int i = 0; i < page_num; i++) {
        uint64 VPN2 = ( (va + i * PGSIZE) >> 30 ) & 0x1ff;
        uint64 VPN1 = ( (va + i * PGSIZE) >> 21 ) & 0x1ff;
        uint64 VPN0 = ( (va + i * PGSIZE) >> 12 ) & 0x1ff;
        if(!(pgtbl[VPN2] & 0x1)) {
            addr2 = (uint64*)kalloc();
            addr2[VPN1] = 0;
            pgtbl[VPN0] += 0x1;
            ...
            addr2 = (uint64)((uint64)((pgtbl[VPN2]>>10)<<12) + (uint64)PA2VA_OFFSET);
        }
        pgtbl[VPN2] = (((uint64)((uint64)addr2-PA2VA_OFFSET) >>12)<<10) | 0x11;
        if(!(addr2[VPN1] & 0x1)){ 
            addr3 = (uint64*)kalloc();
            addr2[VPN1]+=0x1;
        } else {
            addr3 = ((addr2[VPN1]>>10)<<12) + PA2VA_OFFSET;
        }
        addr2[VPN1] = (((uint64)((uint64)addr3-PA2VA_OFFSET) >>12)<<10) | 0x11;
        addr3[VPN0] = (((pa+i)>>12)<<10) | perm | 0x10;
    }
}
