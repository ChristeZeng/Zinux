...
void create_mapping(uint64 *pgtbl, uint64 va, uint64 pa, uint64 sz, int perm) {
   ...
   for(int i = 0; i < page_num; i++) {
        ...
        //printk("\nSecond Page\n");
        uint64 second_page_addr = (read_piece(read_mem((uint64*)first_page_addr), 53, 10) << 12) + 0xff00000000000000 + (vpn1 << 3);
...
void vmtest() {
	...
	uint64 *other2_addr = (uint64*)0x00ffffe000203000;
	uint64 *other_addr = (uint64*)0xffffffe000203000;
