...
void create_mapping(uint64 *pgtbl, uint64 va, uint64 pa, uint64 sz, int perm) {
   ...
   for(int i = 0; i < page_num; i++) {
        ...
        *(uint64*)(spa) = spte;
        ...
        //printk("\nThird Page\n");
        uint64 tpa = ((spte & 0x003FFFFFFFFFFFC00) << 2) /*(read_piece(spte, 53, 10) << 12)*/ + (vpn0 << 3) + PA2VA_OFFSET;
        //printk("third_page_addr = %lx\n", third_page_addr);

        uint64 tpte = (curpa & 0x00FFFFFFFFFFF000) >> 2;//write_piece(tpte, curpa >> 12, 53, 10);
        uint64 tpte = tpte | 0x1 | (perm << 1);//write_piece(tpte, 0x1, 0, 0);
