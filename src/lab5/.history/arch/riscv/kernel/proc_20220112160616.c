...
void create_user_mapping2(uint64 *pgtbl, uint64 va, uint64 pa, uint64 sz, int perm){
    ...
    for(uint64 i = 0; i<sz; i+=PGSIZE){
        uint64 PPN1 = ((va+i)>>30)&0x1ff;
        uint64 PPN2 = ((va+i)>>21)&0x1ff;
        uint64 PPN3 = ((va+i)>>12)&0x1ff;
        if(!(pgtbl[PPN1]&0x1)) {
            addr2 = (uint64*)kalloc();
            addr2[PPN2] = 0;
            pgtbl[PPN1] += 0x1;
        ...
        addr3[PPN3] = (((pa+i)>>12)<<10) | perm | 0x10;
    }
}
...
static unsigned long construct_entry(unsigned long pm_addr, int d, int a, int g, int u, int x, int w, int r, int v) {
...
void task_init() {
    ...
    for(int i = 1; i < NR_TASKS; i++){
        ...
        //权限设置有冲突
        // create_user_mapping(task[i]->pgd, USER_START, (uint64)uapp_start - PA2VA_OFFSET, (uint64)uapp_end - (uint64)uapp_start, 1, 1, 1);
        // create_user_mapping(task[i]->pgd, USER_END - PGSIZE, (uint64)kalloc() - PA2VA_OFFSET, PGSIZE, 1, 1, 1);

        create_mapping_user(task[i]->pgd, USER_START, (uint64)uapp_start - PA2VA_OFFSET, (uint64)uapp_end - (uint64)uapp_start, 7);
        create_mapping_user(task[i]->pgd, USER_END - PGSIZE, (uint64)kalloc() - PA2VA_OFFSET, PGSIZE, 7);
