...
void task_init() {
    ...
    for(int i = 1; i < NR_TASKS; i++){
        ...
        //权限设置有冲突
        create_mapping_user(task[i]->pgd, USER_START, (uint64)uapp_start - PA2VA_OFFSET, (uint64)uapp_end - (uint64)uapp_start, 0b11111);
        create_mapping_user(task[i]->pgd, USER_END - PGSIZE, (uint64)kalloc() - PA2VA_OFFSET, PGSIZE, 0b11111);
