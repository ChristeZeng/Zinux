...
void create_mapping(uint64 *pgtbl, uint64 va, uint64 pa, uint64 sz, int perm) {
   ...
   for(int i = 0; i < page_num; i++) {
        ...
        *(uint64*)((uint64)pgtbl + (vpn2 << 3)) = fpte;
        ...
        //printk("\nSecond Page\n");
        uint64 spa = ((fpte & 0x0000007FFFFFF000) << 2) + (vpn1 << 3) + PA2VA_OFFSET;
        ...
        if(!(spte & 0x1)) {
            ...
            uint64 content = kalloc() - PA2VA_OFFSET;
            //printk("content = %lx\n", content);
            spte = (content & 0x00FFFFFFFFFFF000) >> 2;//write_piece(second_page_entry, content >> 12, 53, 10);
            spte = spte | 0x1;//write_piece(second_page_entry, 0x1, 0, 0);
        ...
        //printk("\nThird Page\n");
        uint64 third_page_addr = (read_piece(read_mem((uint64*)second_page_addr), 53, 10) << 12) + (vpn0 << 3) + PA2VA_OFFSET;
